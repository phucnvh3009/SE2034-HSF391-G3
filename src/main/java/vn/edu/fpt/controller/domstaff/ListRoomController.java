...
public class ListRoomController {
    ...
    public String listRooms(@RequestParam(value = "keyword", defaultValue = "") String keyword,
        ...
        Page<RoomListDTO> roomPage = roomService.getAllRooms(keyword, status, page, 10, currentStaff.getBuilding().getId());
    ...
    public String viewRoomDetail(@PathVariable("id") Long id, Model model, HttpSession session) {
        User currentStaff = (User) session.getAttribute("currentUser");
        if (currentStaff == null) {
            return "redirect:/login";
        }
